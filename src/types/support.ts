import { CategoryChannel, ColorResolvable, Guild, Message, MessageActionRow, MessageButton, MessageComponentInteraction, MessageEmbed, MessageSelectMenu, SelectMenuInteraction, TextChannel } from "discord.js";
...
import { TournaBotGuild } from "./guild";
...
export namespace SupportManager {
    ...
    export const create = async (discordGuild: Guild, discordId: string, supportCategory: CategoryChannel) => {
        ...
        const ticketChannel = await discordGuild.channels.create(`ticket-${ticketId}`, {
            type: 'GUILD_TEXT',
    ...
    export const buildSupportCategory = async (guild: TournaBotGuild, discordGuild: Guild) => {
        ...
        if (!(clientSupportCategory instanceof CategoryChannel)) {
            clientSupportCategory = await discordGuild.channels.create('Support Tickets', {
                type: 'GUILD_CATEGORY',
                permissionOverwrites: [{
                    id: discordGuild.id,
                    deny: ['MANAGE_CHANNELS', 'VIEW_CHANNEL', 'SEND_MESSAGES'],
    ...
    export const buildSupportDashboard = async (guild: TournaBotGuild, discordGuild: Guild, supportCategory: CategoryChannel) => {
        ...
        if (!(clientSupportDashboard instanceof TextChannel)) {
            clientSupportDashboard = await discordGuild.channels.create('support-dashboard', {
                type: 'GUILD_TEXT',
                permissionOverwrites: [{
                    id: discordGuild.id,
                    deny: ['MANAGE_CHANNELS', 'VIEW_CHANNEL', 'SEND_MESSAGES'],
...
class SupportTicket implements SupportTicketData {
    ...
    async requestAdmin(m: Messenger, supportDashboard: TextChannel, embed: MessageEmbed) {
    ...
    async getRequestEmbed(m: Messenger, set?: SupportSet) {
        ...
        const embed = new MessageEmbed()
            ...
            .addField(await m.translate('Issue'), this.issue.name, true)
            .addField(await m.translate('User'), `<@${user.id}>`, true)
            .addField(await m.translate('Ticket'), `<#${this.channelId}>`, true)
            .addField(await m.translate('Status'), await m.translate(this.resolved ? '\`RESOLVED\` :white_check_mark:' : '\`NOT RESOLVED\` :x:'), true)
        ...
        if (set) {
            ...
            embed.addField(await m.translate('Matchup'), `**${entrantOne.name}** ${entrantOne.mentions.join('')} vs. **${entrantTwo.name}** ${entrantTwo.mentions.join('')}`, true)
                .addField(await m.translate('Set'), `[Set Link](${set.url})`, true)
        }
    ...
    async sendSetSelectMessage(m: Messenger, response: Message | SelectMenuInteraction, supportSets: SupportSet[]) {
        ...
        const noneButton = new MessageButton()
            .setCustomId(`support_set_none`)
            .setStyle('SECONDARY')
            .setLabel(`None`);

        actionRows.push(new MessageActionRow().addComponents(noneButton));
    ...
    async resolve(m: Messenger, interaction: MessageComponentInteraction | SelectMenuInteraction) {
        ...
        await interaction.update({ embeds: [await this.getRequestEmbed(m, this.setId ? await this.getSupportSet() : null)], components: [actionRow] });
        await this.updateAll();
    }
