import { Guild, MessageActionRow, MessageButton, MessageComponentInteraction, MessageEmbed, MessageSelectMenu, OverwriteResolvable, Snowflake, TextChannel } from "discord.js";
import ShortUniqueId from "short-unique-id";
import { client, guilds } from "../main";
import { Messenger } from "../messaging";
import { TournaBotDiscordData, TournaBotGuild } from "./guild";
...
export class MatchmakingQueue {
    ...
    async buildQueueChannel(clientGuild: Guild, guild: TournaBotGuild, interactionChannel?: TextChannel) {
        ...
        if (!(queueChannel instanceof TextChannel)) {
            queueChannel = await clientGuild.channels.create(this.name, {
                type: 'GUILD_TEXT',
    ...
    async buildQueuePanel(channel: TextChannel) {
        ...
        const embed = new MessageEmbed()
            ...
            .addField(await m.translate('Players Needed To Start Game'), `**${(this.game.playersPerGame - this.playersInQueue.length).toString()}**`)
...
export class MatchmakingMatch implements MatchmakingMatchInterface {
    ...
    async createChannels(clientGuild: Guild) {
        const category = await clientGuild.channels.create(`${this.queueName} Match`, {
            type: 'GUILD_CATEGORY',
            permissionOverwrites: [{
                id: clientGuild.id,
        ...
        const adminChannel = await clientGuild.channels.create('admin', {
            type: 'GUILD_TEXT',
            parent: category
        });
        ...
        const gameChannel = await clientGuild.channels.create('match', {
            type: 'GUILD_TEXT',
        ...
        const chatChannel = await clientGuild.channels.create('chat', {
            type: 'GUILD_TEXT',
    ...
    phases = {
        checkIn: async (send: boolean = false, clientGuild: Guild, interaction?: MessageComponentInteraction) => {
            ...
            const embed = new MessageEmbed()
                ...
                .addField(await m.translate('Not Checked-in'), content)
                .setColor(`#${process.env.EMBED_COLOR}`)
                .setFooter('TournaBot', client.user.avatarURL({ format: 'png' }))
                .setTimestamp();
