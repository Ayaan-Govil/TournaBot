    ...
    async iterate() {
        ...
        for (const event of events) {
            for (const set of event.sets.nodes) {
                ...
                await this.channel.send(await this.generateMatchupMessage(event.slug, set));
            }
        }
    }

    async generateMatchupMessage(slug: string, set: Set) {
        const entrants = await getEntrants(set.slots);

        const embed = new MessageEmbed()
            .setAuthor({ name: `${set.event.name} - ${set.fullRoundText} - Best of ${set.totalGames}` })
            .setTitle(`${entrants[0].name} vs. ${entrants[0].name}`)
            .setURL(`https://start.gg/${slug}/set/${set.id}`)
    ...
    async iterate() {
        ...
        for (const event of events) {
            for (const set of event.sets.nodes) {
                if (this.sets.includes(set.id)) continue;
                this.sets.push(set.id);
                await this.channel.send(await this.generateResultsEmbed(event.slug, set));
            }
        }
    }

    async generateResultsEmbed(slug: string, set: Set) {
        const entrants = await getEntrants(set.slots);
