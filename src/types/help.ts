...
export namespace HelpStructure {
            ...
            features: {
                ...
                support: {
                    ...
                    commands: [
                        {
                            command: 'support',
                            description: 'Creates a support ticket for you to address an issue related to the tournament. You can talk with TournaBot to identify your issue or select your issue from the select menu.',
                            image: ''
                        }
                    ]
                }
            ...
            features: {
                    ...
                    commands: [
                        {
                            ...
                            description: 'Sets the active tournament for the server. Features such as match reporting/calling and support tickets require this command.',
                            image: ''
                        },
                        {
                            command: 'tournament announce <message?>',
                            description: 'Announces a tournament with tournament and event info. If the `message` argument is omitted, TournaBot will automatically generate a message. You can set the role to ping in the automatic message using `/set role`.',
                            image: ''
                        }
                    ]
                },
                support: {
                    ...
                    commands: [
                        {
                            command: 'support',
                            description: 'You must set a tournament using `/tournament set` to use this command. Creates a support ticket. Generates a category and dashboard for tickets when initially ran. TournaBot will identify the user\'s issue and set before sending a request for an admin in the dashboard.',
                            image: ''
                        }
                    ]
                },
                    ...
                    commands: [
                        {
                            ...
                            description: 'You must set a channel and tournament using `/set channel` and `/tournament set` to use this command. Pings users for their matches as they are called. Gives round information and links to the set. Needs a Discord connection via `/account link` or start.gg to work effectively. Match calling can be manually stopped using `stop`, but will automatically stop after 6 hours or when the tournament ends according to the tournament details.',
                            image: ''
                        },
                        {
                            command: 'match reporting <start/stop>',
                            description: 'You must set a channel and tournament using `/set channel` and `/tournament set` to use this command. Reports match outcomes as they finish. Gives details such as the seeds, games, characters picked, and whether it was an upset or not. Needs a Discord connection via `/account link` or start.gg to work effectively. Match reporting can be manually stopped using `stop`, but will automatically stop after 6 hours or when the tournament ends according to the tournament details.',
                            image: ''
                        }
                    ]
                },
                    ...
                    commands: [
                        {
                            ...
                            description: 'Adds a role to the allowlist. Allows that role to use admin commands.',
                            image: ''
                        },
                        {
                            command: 'allowlist remove <role>',
                            description: 'Removes a role from the allowlist.',
                            image: ''
                        }
                    ]
                }
        ...
        if (feature.length) {
            ...
            for (const feature in typeObj.features) {
                if (countIndex >= min && countIndex < max) {
                    const featureObj = typeObj.features[feature];
                    embed.description += `:small_orange_diamond:**${capitalize(feature)}**\n\n${featureObj.longDescription}\n\n`;
                }
                countIndex++;
            }

            const otherType = type === 'user' ? 'admin' : 'user';
            embed.description += `**You can the select menu below to switch to ${otherType} features.**`;
        }
    ...
    export const getComponents = (type: string, pageIndex: number, feature: string = '') => {
        ...
        if (!feature.length) {
            ...
            actionRows.push(new MessageActionRow().addComponents(selectMenu));
        }
